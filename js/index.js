const Game = (function() {
	...
	return {
		...
		save: function() {
			...
			let values = JSON.parse(localStorage.getItem('game'));
			if (values === null) {
				values = [];
			}
			...
			this.addToTable(values.length - 1, saveObject.name, saveObject.gameScore);
			this.close();
		},

		addToTable: function(index, name, gameScore) {
		...
		renderTableScore: function() {
			...
			data.map((item, index) => this.addToTable(index, item.name, item.gameScore));
		},
